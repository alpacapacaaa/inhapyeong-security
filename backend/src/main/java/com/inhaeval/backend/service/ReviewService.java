...
public class ReviewService {
    ...
    @Transactional
    public Long createReview(ReviewRequest request, Long memberId) {
    ...
    public List<Review> getReviewsByCourseId(Long courseId, String sortBy) {
        
        // 프론트에서 넘어온 "sortBy" 글자에 따라 우리가 아까 Repository에 심어둔 필살기 쿼리 중 하나를 고릅니다.
        if ("highest".equalsIgnoreCase(sortBy)) {
            return reviewRepository.findByCourseIdOrderByRatingDesc(courseId); // 별점 높은 순
        } else if ("lowest".equalsIgnoreCase(sortBy)) {
            return reviewRepository.findByCourseIdOrderByRatingAsc(courseId);  // 별점 낮은 순
        } else if ("likes".equalsIgnoreCase(sortBy)) {
            return reviewRepository.findByCourseIdOrderByLikesCountDesc(courseId); // 추천 많은 순
        } else {
            return reviewRepository.findByCourseIdOrderByCreatedAtDesc(courseId); // 기본값: 최신순
        }
    }
}
